...
@section('content')
    ...
    <div style="overflow-x: auto;">
        <table class="fl-table">
            ...
            <tbody>
            @foreach($commandes as $commandeImpayes)
                @if ($commandeImpayes->facturee ===0)
                    <tr>
                        <td>{{$commandeImpayes->noCommande}}</td>
                        <td>{{$commandeImpayes->entCli}}</td>
                        <td>{{$commandeImpayes->pxttc}}</td>
                        <td>{{$commandeImpayes->mpaiement}}</td>
                        <td>{{$commandeImpayes->dateCommande}}</td>
                        <td>
                            <a href="">
                                @if ($commandeImpayes->valClient === 0)
                        ...
                        <td>{{$commandeImpayes->dateExpd}}</td>
                        <td>
                            <a href="">
                                @if ($commandeImpayes->facturee === 0)
                                    <svg style="color: red" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-check2" viewBox="0 0 16 16">
                        ...
                        <td>{{$commandeImpayes->nomPdf}}</td>
                        <td>
                            <a href="">
                                @if ($commandeImpayes->envoyee === 0)
                                    <svg style="color: red" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-check2" viewBox="0 0 16 16">
                                        <path d="M13.854 3.646a.5.5 0 0 1 0 .708l-7 7a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L6.5 10.293l6.646-6.647a.5.5 0 0 1 .708 0z"/>
                                    </svg>
                                @else
