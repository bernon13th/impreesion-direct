...
@section('content')
    ...
    <div style="overflow-x: auto;">
        <table class="fl-table">
            ...
            <tbody>
            @foreach($commandes->data as $commandeAExpedier)
                @if ($commandeAExpedier->expediee ===0)
                    <tr>
                        <td>{{$commandeAExpedier->noCommande}}</td>
                        <td>{{$commandeAExpedier->entCli}}</td>
                        <td>{{$commandeAExpedier->pxttc}}</td>
                        <td>{{$commandeAExpedier->mpaiement}}</td>
                        <td>{{$commandeAExpedier->dateCommande}}</td>
                        <td>
                            <a href="">
                                @if ($commandeAExpedier->valClient === 0)
                        ...
                        <td>
                            <a href="">
                                @if ($commandeAExpedier->validee === 0)
                                    <svg style="color: red" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-check2" viewBox="0 0 16 16">
                                        <path d="M13.854 3.646a.5.5 0 0 1 0 .708l-7 7a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L6.5 10.293l6.646-6.647a.5.5 0 0 1 .708 0z"/>
                                    </svg>
                                @else
                        ...
                        <td>{{$commandeAExpedier->dateExpd}}</td>
                    </tr>
                @endif
            @endforeach
            </tbody>
